...
- Desktop 使用 SQLite `CanvasRepo`；页面编辑后约 420 ms 防抖保存。通过白板顶部返回入口或进入完整阅读器时，路由跳转会等待当前工作区的最新快照写入成功；写入失败会留在原页面并允许重试。
- 切换白板前会 flush 当前工作区；延迟保存和异步加载都以 `workspaceId` 分区或校验，避免旧白板的操作覆盖新白板。
- 关闭桌面窗口或退出应用时，Electron 主进程会先拦截关闭并向渲染进程发送带唯一请求 ID 的保存请求。渲染进程先提交卡片、连线与专注 Markdown 编辑器里的活动草稿，再停止接受新的白板文档变更，清除全部防抖计时器、等待既有写入结束并 flush 所有已载入工作区，最后把结果回传主进程；输入法仍在组词、草稿冲突或写入失败时会保留窗口。重复关闭请求会合并，回包只对当前请求生效；超时取消还会终止尚未进入的后续保存屏障，避免旧请求重新锁住画布。保存失败默认保持窗口并提供重试路径，只有用户明确确认才会强制关闭；主进程等待超时时同样不会自动丢弃修改。页面 `pagehide` 与卸载清理仍保留为 best-effort 兜底，但不再承担桌面关闭时的可靠性保证。
