...
专注编辑器会把尚未提交的标题和 Markdown 正文按 `workspaceId + nodeId` 自动保存为当前设备上的恢复草稿。刷新页面、意外关闭或应用异常退出后，再次打开对应笔记时可恢复最后的本机草稿；恢复态仍被视为未保存修改，而不是新的持久化基线。关闭专注编辑器，或在其有改动时通过应用路由切换白板/离开空间白板，会统一提供「保存并关闭」「放弃修改」「继续编辑」三种选择。保存和恢复都会复核白板、节点以及开始编辑时的内容版本；节点被删除、白板已切换或持久化内容已经更新时，系统会保留草稿并明确提示冲突，不会静默覆盖较新的笔记。
...
选择同一层级的至少两张卡片后，可从选区浮条、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 打开自动整理菜单。“整齐排列”支持文献、摘录、研究笔记、AI 合成与分组容器混选，按选区原有的上到下、左到右阅读顺序，以真实卡片尺寸生成不重叠的紧凑网格；根层分组作为整体参与布局，分组与其子卡片同时被选中时只移动分组，避免重复位移。同一展开分组内的混合子卡也可整理，容器仅在需要时扩大。根节点与不同分组内的子节点不会跨坐标系混排，也不会自动改变分组归属。
...
6. 点击 Idea Note 的标题或正文即可在卡片内编辑；需要更大空间时可展开带源码、分屏、预览与 Markdown/LaTeX 工具的专注编辑器。专注编辑器会在当前设备自动保存恢复草稿；刷新或异常退出后重新打开笔记即可恢复。存在未保存修改时关闭或离开会提供「保存并关闭」「放弃修改」「继续编辑」，版本冲突时保留草稿而不覆盖新内容。普通单击 AI 合成或分组仍打开左侧“详情与编辑”面板；单击连线只选择它，双击连线或选中后按 `F2` 可就地编辑自由文字。
...
15. 选择同一层级的至少两张卡片，通过选区浮条、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 做混选网格整理；如果全部是文献卡，还可选择年份时间轴或引用树。引用树会从 Library、引文脉络缓存或 OpenAlex 解析所选文献之间的关系，循环引用节点排列在同一列。
...
当前交互只支持单层分组，新的嵌套 Group 快照会在保存边界被拒绝。早期版本曾允许这类数据落库；读取旧快照时会把嵌套 Group 的相对位置累加为根坐标并解除其父分组，普通子卡仍保持相对所属 Group 的坐标，因此升级不会让既有白板不可打开或发生视觉跳位。下一次保存会自然写回规范化后的扁平结构。

### 5.2 Desktop 与浏览器预览

- Desktop 使用 SQLite `CanvasRepo`；页面编辑后约 420 ms 防抖保存。
- 所有由 React Router 管理的离开动作（顶部返回、全局导航、完整阅读器、白板切换以及前进/后退）统一经过空间白板页面唯一的导航屏障。屏障先顺序完成卡片内联编辑、连线标签与专注 Markdown 草稿的离开准备，再 flush 所有已载入工作区；若准备期间活动编辑器集合发生变化，则重新准备并 flush，直到状态稳定后才放行。输入法仍在组词、用户选择继续编辑、版本冲突或写入失败都会取消导航并停留原页面。
- 延迟保存、异步加载与编辑器提交都以 `workspaceId` 分区或校验，避免旧白板操作覆盖新白板。
- 关闭桌面窗口或退出应用时，Electron 主进程会先拦截关闭并向渲染进程发送带唯一请求 ID 的保存请求。渲染进程复用与路由导航相同的活动编辑器准备注册表，依次提交卡片内联编辑、连线标签与专注 Markdown 草稿，再停止接受新的白板文档变更，清除全部防抖计时器、等待既有写入结束并 flush 所有已载入工作区，最后把结果回传主进程；输入法仍在组词、草稿冲突或写入失败时会保留窗口。重复关闭请求会合并，回包只对当前请求生效；超时取消还会终止尚未进入的后续保存屏障，避免旧请求重新锁住画布。保存失败默认保持窗口并提供重试路径，只有用户明确确认才会强制关闭；主进程等待超时时同样不会自动丢弃修改。页面 `pagehide` 与卸载清理仍保留为 best-effort 兜底，但不再承担桌面关闭时的可靠性保证。
- 专注笔记编辑器另有设备本地恢复层：草稿在逻辑上以 `workspaceId + nodeId` 隔离，并以浏览器窗口 owner 与不可变 revision 分开存储，避免同一笔记在多个标签页中互相覆盖或误删较新的草稿；记录包含标题、Markdown 正文、基础内容版本和草稿更新时间。成功提交或明确放弃后只解析对应的精确 revision；节点/白板错配、版本冲突或写入失败都会保留草稿并阻止静默覆盖。草稿不属于 `CanvasWorkspaceDocument`，不写入 SQLite，也不进入整库 JSON 备份或 WebDAV 同步。
...
操作系统强制终止进程、断电或 `SIGKILL` 无法等待异步保存；这些场景不属于关闭握手能够承诺的范围。
...
导入边界会先拒绝重复节点 ID、空白标识、跨工作区分组引用及其他无法保持身份的数据；旧备份中的嵌套 Group 使用与数据库读取相同的坐标规则安全扁平化。三个 Canvas 表使用严格插入并共享整库导入事务，任何约束失败都会回滚整个导入，不会因静默跳过父 Group 而留下孤立节点或部分白板。

当前 WebDAV 行级同步**不包含空间白板表**；空间白板跨设备迁移目前依赖整库 JSON 导出/导入。设备本地的笔记恢复草稿也不会通过 WebDAV 同步或随整库 JSON 迁移；只有用户正式保存到白板文档后的笔记内容才会进入后续备份范围。设置页也明确展示这一范围。
...
- 专注笔记草稿只用于当前设备上的异常恢复，不是跨设备同步或长期版本管理；草稿暂不进入 WebDAV 与整库 JSON 备份。
...
- 通用网格整理作用于同一层级的至少两张卡片，可混选全部内置类型；跨层级选择不会自动改组或混排。时间轴与引用树仍只适用于纯文献选择；引用树依赖 Library/引文脉络/OpenAlex 中可识别的 DOI 或本地 `cites` 数据。无 DOI 且 Library 尚无关系的文献仍可使用网格或时间轴整理；普通手工连线不会被当作学术引文。
...
- 混选网格、时间轴与引用树整理：`packages/core/src/canvas/layout.ts`
