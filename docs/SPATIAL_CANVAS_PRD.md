...
画布基于 `@xyflow/react` 实现，提供无限点阵背景、缩放和平移、框选与多选、节点拖动、关系连线、可折叠逻辑分组，以及单一底部浮动功能栏。这个 Dock 除了选择、平移、缩放、新建笔记、分组、连接和 AI 合成，也统一承载“文献库”“详情与编辑”“画布导航”三个面板入口；入口共用一个从左侧展开的内容面板，不再保留独立左侧工具条或旧右侧检查器。导航面板包含内容统计、显示全部和 MiniMap；MiniMap 只在该面板实际打开时挂载，避免隐藏容器的尺寸观测触发 `ResizeObserver` 抖动。
...
4. 从底部统一功能栏打开左侧“文献库”面板，点击或拖入任意文献，可在当前白板创建 `PaperNode`；同一功能栏还能打开详情编辑和画布导航。
...
- 只有从底部功能栏打开左侧“画布导航”面板时才挂载 React Flow `MiniMap`；收起或切换面板会直接卸载它，避免对不可见容器持续尺寸观测。
