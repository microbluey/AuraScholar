...
7. 保持“选择”模式，悬停或聚焦任意卡片即可显示四向磁力连接点。从来源卡拖到目标卡后，原地弹出 **Cites 引用**、**Supports 支持**、**Contradicts 反驳**、**Extends 扩展** 四个语义 Pills；点击选项或按数字键 `1`–`4` 提交，按 `Escape` 或点击气泡外取消。选择语义前不会写入永久关系。
8. 同一来源 → 目标方向已经存在关系时不会重复创建，而目标 → 来源的反向关系仍可独立建立。
9. 选择多张卡片后可放入分组，或从文献/摘录生成 AI 合成卡。
10. 新建 Idea Note，在画布中记录 Markdown 与 LaTeX 研究笔记。
11. 删除画布卡片只删除当前白板中的摆放及其关系，不会删除原文献或原批注。
...
快捷语义气泡只展示前四种研究关系，并按 `1`–`4` 固定映射；`derived-from` 继续由摘录与 AI 合成流程自动创建，`custom` 则可在关系检查器中编辑。同一来源和目标方向最多保留一条边，但反向边被视为不同的有效关系。
