...
专注编辑器会把尚未提交的标题和 Markdown 正文按 `workspaceId + nodeId` 自动保存为当前设备上的恢复草稿。刷新页面、意外关闭或应用异常退出后，再次打开对应笔记时可恢复最后的本机草稿；恢复态仍被视为未保存修改，而不是新的持久化基线。关闭编辑器、切换白板或离开空间白板时，如果内容有变化，统一提供「保存并关闭」「放弃修改」「继续编辑」三种选择。保存和恢复都会复核白板、节点以及开始编辑时的内容版本；节点被删除、白板已切换或持久化内容已经更新时，系统会保留草稿并明确提示冲突，不会静默覆盖较新的笔记。
