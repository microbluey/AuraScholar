...
画布基于 `@xyflow/react` 实现，提供无限点阵背景、缩放和平移、框选与多选、节点拖动、关系连线和可折叠逻辑分组。控件按作用域分层：底部 Dock 只保留“文献库”“新建”和“指针”，缩放、恢复 100%、显示全部和按需挂载的 MiniMap 位于右下导航岛；分组、自动整理、AI 合成和批量移除仅在多选后显示于选区上方。详情没有常驻入口，节点与关系的普通单击直接打开对应主工作表面。

普通单击直接进入节点的主要工作表面：文献和摘录打开与 React Flow 视口并列的右侧阅读器，研究笔记、AI 合成和分组打开左侧详情编辑面板，关系边也通过单击进入同一面板编辑。桌面端阅读器默认约按白板 60%、阅读器 40% 分屏，分隔条可拖动或用键盘调整；阅读器不是画布变换层的子元素，因此缩放和平移白板不会缩放阅读器。鼠标右键、触摸板双指点击或卡片 `...` 按钮会打开节点上下文菜单；`Shift` / `Cmd` / `Ctrl` + 单击只修改多选，不触发内容打开。“选择”模式不再把右键用于平移，用户仍可通过鼠标中键、按住 `Space` 或切换“平移”工具拖动画布。
...
选择同一层级的至少两张 `PaperNode` 后，可从选区浮条、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 打开自动整理菜单：时间轴按发表年份排列，引用树则按所选节点间的 `cites` 关系排列。引用树只在所选文献间至少存在一条 `cites` 关系时可用；循环引用会作为同一强连通分量保留在同一列，避免循环导致布局失败。根节点与分组内节点不会混合排布。
...
6. 普通单击 Idea Note、AI 合成或分组会打开左侧“详情与编辑”面板；单击关系边会在该面板编辑关系类型与说明。
...
9. 悬停或聚焦任意卡片即可显示四向磁力连接点。从来源卡拖到目标卡后，原地弹出 **Cites 引用**、**Supports 支持**、**Contradicts 反驳**、**Extends 扩展** 四个语义 Pills；拖到空白处则先显示可搜索的目标卡片选择器和虚线预览。目标可按标题、元数据、标签、所属分组与节点类型检索；选择后才进入语义 Pills。按 `Escape`、点击外部、删除端点或切换白板均取消，选择语义前不会写入永久关系。
...
快捷语义气泡只展示前四种研究关系，并按 `1`–`4` 固定映射；`derived-from` 继续由摘录与 AI 合成流程自动创建，`custom` 则可在左侧“详情与编辑”面板中编辑。同一来源和目标方向最多保留一条边，但反向边被视为不同的有效关系。
