...
- **多个独立白板**:可将不同研究项目分开到独立工作区;画布顶部切换器支持新建、切换与重命名,打开 `/canvas` 会恢复最近使用的白板,并进入 RESTful 路由 `/canvas/:workspaceId`。旧版默认白板数据会作为第一个工作区保留;当前尚未提供归档和删除入口。
...
- **关系与来源回溯**:用引用、支持、反驳、扩展和自定义关系组织材料;文献卡可打开阅读器,摘录卡可返回对应附件、批注与页码。
...
- **本地持久化**:桌面端将白板保存到 SQLite,整库 JSON 备份包含白板数据;空间白板暂未纳入 WebDAV 行级同步。详见[空间白板产品与架构说明](./docs/SPATIAL_CANVAS_PRD.md)。
