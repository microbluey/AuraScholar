...
- **可安全回滚的文献状态与回收站**:重点/阅读状态以及单篇或批量移入、恢复都通过类型化主进程命令执行;每个批次都是单个 Library 级事务,不会只成功一部分。永久删除复用同一归属防线,任一步失败都不会留下“删了一半”的数据。未被引用的内容寻址 Blob 文件暂不立即回收,后续会交给全局串行的清理器处理。
