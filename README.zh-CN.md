...
- **可安全回滚的文献状态与回收站**:重点/阅读状态以及单篇或批量移入、恢复都通过类型化主进程命令执行;每个批次都是单个 Library 级事务,不会只成功一部分。永久删除复用同一归属防线,任一步失败都不会留下“删了一半”的数据。未完成的暂存导入遗留的 PDF 内容寻址 Blob 会由主进程在容量与时间边界内清理;清理前会检查全部附件和文档修订引用,不会扫描或误删任意历史 Blob。
- **关系安全的重复文献合并**:重复文献会在主进程的单个 Library 级事务中合并;有效文件夹与标签归属取并集,PDF、批注、引文、Canvas 引用、衍生数据与后台任务都会迁移到主记录,任一步失败都会完整回滚。
- **元数据来源**:聚合 Crossref、OpenAlex、Semantic Scholar、Unpaywall、arXiv 五个开放数据源。

### 🗂️ 研究项目

- **有边界的项目工作区**:可新建、切换和重命名独立研究项目,并使用 RESTful 路由 `/projects/:projectId`;打开 `/projects` 会恢复最近使用的活跃项目,首次使用则建立安全的默认项目范围。
- **不复制文献的项目来源**:可在项目内检索整个文献库并批量加入来源;文献库单篇与批量操作复用轻量目标选择器。只有一个项目时直接加入,多个项目时默认选中最近活跃项,也可就地新建。
- **安全的成员关系**:从项目移除来源只解除项目归属,绝不会删除文献库中的主文献、元数据、批注、PDF 附件与源文件。

### 🔍 学术检索

- **开放源聚合检索**:OpenAlex / Crossref / Semantic Scholar / arXiv 原生聚合,结果去重合并、标记是否已在库,一键入库(含 OA PDF 获取)。
- **保存检索订阅**:保存开放源检索后,桌面应用启动时会补跑;桌面窗口保持打开时,每小时调度器会检查到期订阅。关闭应用后不会在后台或 7×24 小时继续轮询。
- **可靠的全文补全闭环**:从文献库、阅读器或检索结果发起“找全文”时,系统会先按 Unpaywall → arXiv → 明确 OA PDF 线索逐个验证,失败后再进入站点浏览。每个任务携带唯一 `handoffId`、目标文献和来源页面;Reader/Library 任务另带受校验的返回地址,浏览器回退则绑定实际根标签及其子标签链。异步下载不会串到后来打开的另一篇文献,安全返回地址存在时会在挂载成功并刷新全文状态后返回。
- **内置学术浏览器**:在应用内多标签打开 Google Scholar、Web of Science、Scopus、PubMed、CNKI、IEEE Xplore、ScienceDirect、SpringerLink、Wiley、ACM、JSTOR、ResearchGate、bioRxiv、DBLP、百度学术、万方、维普等常用站点(可增删自定义站点)。每个站点登录态独立隔离并持久保存。
  - **下载捕获与确认**:在站内(含机构订阅)下载的 PDF 会被自动捕获与解析。新文件或 DOI/内容哈希跨文献冲突会等待用户确认目标;若明确命中当前目标文献,则可幂等挂载。导出的引用文件仍会自动识别并按标识去重导入。
...
- 阅读器选区可保存为绑定精确修订的 PDF 文本 Evidence,并可回到捕获时的来源修订。
- 面板视图:批注 · 翻译 · 引文脉络(后者仅适用于带 DOI 的文献)。
...
- 自动监控论文从 Accept → Online → 正式出版 → 数据库收录的全过程,状态变化即时通知,并保存证据快照;无 DOI 的论文支持按标题监控,命中后自动升级为 DOI 跟踪,出版后自动入库。桌面应用启动时会补跑,且仅在桌面窗口保持打开期间每小时轮询;关闭应用后不会在后台或 7×24 小时持续监控。
...
- **WebDAV 行级同步(当前范围)**:可通过自己的 WebDAV 服务(含兼容 NAS 或网盘)同步文献、研究项目及其成员关系、文档资产/修订元数据、Evidence 及其项目归属、批注、闪卡和哨兵任务状态(混合逻辑时钟 + 逐字段 LWW 冲突解决)。
- **本机来源文件与空间白板**:WebDAV 不传输来源文档 Blob,包括 PDF;远端修订需要显式重新挂载。作者、文件夹或标签关系、写作素材、保存检索和空间白板也不在当前 WebDAV 同步范围内。空间白板目前通过整库 JSON 导出/导入迁移,该备份同样不包含 PDF 二进制。AI 使用你自己的模型服务与 API Key(OpenAI 兼容 / Anthropic)。
...
Web 与移动端目前仅处于规划阶段,仓库中尚未创建对应的应用目录。
