...
- **可安全回滚的回收站**:移入回收站后仍可恢复;永久删除会在主进程中以单个 Library 级事务清理文献及关联记录,任一步失败都不会留下“删了一半”的数据。未被引用的内容寻址 Blob 文件暂不立即回收,后续会交给全局串行的清理器处理。
...
- **可恢复的本机笔记草稿**:专注 Markdown 编辑器会按白板、节点与窗口 revision 在当前设备自动保存草稿,刷新页面或异常退出后仍可恢复尚未提交的研究笔记,多个标签页也不会互相覆盖。存在修改时关闭编辑器,或通过应用路由离开当前页面,会提供「保存并关闭」「放弃修改」「继续编辑」三种选择;恢复和保存都会重新校验白板、节点与基础内容版本,不会静默覆盖更新后的笔记。这类恢复草稿仅保存在本机,暂不通过 WebDAV 同步。
...
- **高亮拖入白板**:保存 PDF 高亮后,可把摘录条拖到白板,也可点击「加入当前白板」。系统会先原子校验数据库中的批注、附件、文献与目标白板身份,再创建或复用 `PaperNode`、创建 `ExcerptNode` 并建立来源连线。重复加入会定位既有摘录,并按需补回来源文献卡或连线而不复制内容;旧请求、身份冲突或提交失败都不会留下半成品。
...
- **关闭安全的本地持久化**:桌面端将白板保存到 SQLite。所有由应用路由管理的离开动作(切换白板、全局导航、前进/后退等)都会先处理卡片内联编辑、连线文字与专注 Markdown 草稿,再等待所有已载入白板的最新快照落库;输入法仍在组词、用户选择「继续编辑」、版本冲突或保存失败都会取消跳转并停留当前白板。关闭窗口或退出应用时还会通过渲染进程与主进程之间的限时保存握手确认写入完成;保存失败默认保留当前窗口,只有用户明确确认才会强制关闭。整库 JSON 备份包含白板数据,空间白板暂未纳入 WebDAV 行级同步。浏览器预览有意不读取本机 PDF,同屏阅读的真实 PDF 工作流需在桌面应用中使用。详见[空间白板产品与架构说明](./docs/SPATIAL_CANVAS_PRD.md)。
