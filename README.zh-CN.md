...
- **无限研究画布**:把完整文献、PDF 摘录、研究想法与 AI 合成结果放入同一张可缩放、可平移的点阵画布;支持框选、多选、拖动、关系连线与可折叠分组。
...
- **直接操作卡片**:普通单击文献或摘录会打开同屏阅读器,单击研究想法、AI 合成或分组会进入左侧详情面板,单击连线则进入关系编辑;鼠标右键、触摸板双指点击或卡片 `...` 可打开上下文操作。`Shift` / `Cmd` / `Ctrl` + 单击仍只用于多选。
- **有保护的键盘删除**:画布本身聚焦时,按 `Delete` / `Backspace` 可移除选中的画布节点或关系;正在输入、使用弹窗/菜单或操作阅读器时快捷键不会生效。删除分组只移除外壳并保留子卡片,移除任何画布卡片都不会删除文献库论文或 PDF 源文件。
...
- **时间轴与引用树整理**:选择至少两张处于同一层级的 `PaperNode`,可通过选区浮条、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 按发表年份或引用关系自动排布。引用树要求所选文献之间存在 `cites` 关系,循环引用的文献会保持在同一列。
...
- **快捷语义连线与来源回溯**:悬停或聚焦卡片即可显示四向磁力连接点;拖到另一张卡片后,在原地 Pills 中选择**引用**、**支持**、**反驳**或**扩展**,也可按数字键 `1`–`4`。若落在空白处,会保留虚线预览并弹出当前白板目标选择器;可按标题、元数据、标签、分组或类型搜索,选择折叠分组内卡片时会先展开再连接。选择语义前、按 `Escape` 取消或切换白板都不会写入关系。同一来源 → 目标方向只保留一条关系,反向关系仍可独立建立。
- **有来源边界的 AI 合成**:选择 2–10 张文献或摘录节点,生成方法论对比、分歧分析、研究空白或简明综述。文献节点只提供题录与可用摘要,而非 PDF 全文;摘录节点提供所选原文。异步结果写入前会重新校验请求、白板以及每个来源节点的 ID、类型与内容版本,旧请求不能生成悬空来源连线。结果保留来源节点与派生关系,实际生成需要配置 AI 服务。
