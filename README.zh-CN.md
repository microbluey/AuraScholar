...
- **关闭安全的本地持久化**:桌面端将白板保存到 SQLite;显式离开白板前会等待最新快照落库,关闭窗口或退出应用时还会通过渲染进程与主进程之间的限时保存握手确认写入完成。保存失败默认保留当前窗口,只有用户明确确认才会强制关闭。整库 JSON 备份包含白板数据,空间白板暂未纳入 WebDAV 行级同步。浏览器预览有意不读取本机 PDF,同屏阅读的真实 PDF 工作流需在桌面应用中使用。详见[空间白板产品与架构说明](./docs/SPATIAL_CANVAS_PRD.md)。
