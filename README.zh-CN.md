...
- **单一画布功能栏**:底部浮动 Dock 统一容纳文献库、详情、概览入口,以及选择、导航、新建、分组、连线和 AI 工具;前三个入口共用一个从左侧展开的内容面板,概览提供内容统计、显示全部和 MiniMap。独立左侧工具条与旧右侧检查器均已移除。
...
- **时间轴与引用树整理**:选择至少两张处于同一层级的 `PaperNode`,可通过 Dock、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 按发表年份或引用关系自动排布。引用树要求所选文献之间存在 `cites` 关系,循环引用的文献会保持在同一列。
...
- **快捷语义连线与来源回溯**:在“选择”模式下悬停或聚焦卡片,即可显示上、下、左、右四个磁力连接点;从一张卡片拖到另一张后,在原地 Pills 中选择**引用**、**支持**、**反驳**或**扩展**,也可按数字键 `1`–`4`,`Escape` 取消。同一来源 → 目标方向只保留一条关系,反向目标 → 来源仍可独立建立;自定义关系可继续在左侧详情面板中修改,摘录卡也会保留来源锚点。“选择”模式下右键专用于卡片菜单,中键、`Space` 或“平移”工具仍可拖动画布。
