...
- **直接操作卡片与连线**:普通单击文献或摘录会打开同屏阅读器,单击研究想法、AI 合成或分组会进入左侧详情面板;单击连线只选择它,双击连线可就地编辑可选的自由文字,键盘可用 `F2`。鼠标右键、触摸板双指点击或卡片 `...` 可打开上下文操作。`Shift` / `Cmd` / `Ctrl` + 单击仍只用于多选。
...
- **时间轴与引用树整理**:选择至少两张处于同一层级的 `PaperNode`,可通过选区浮条、多选右键菜单或 `Cmd` / `Ctrl` + `Shift` + `L` 按发表年份或引文结构自动排布。只有所选文献在当前白板文档中已经带有系统/历史引文数据时才显示引用树;手工连线不会被分类为引文数据。循环引用的文献会保持在同一列。
