...
- **WebDAV 行级同步(当前范围)**:可通过自己的 WebDAV 服务(含兼容 NAS 或网盘)同步文献、研究项目及其成员关系、文档资产/修订元数据、Evidence 及其项目归属、批注、写作素材、闪卡和哨兵任务状态(混合逻辑时钟 + 逐字段 LWW 冲突解决)。
- **本机来源文件与空间白板**:WebDAV 不传输来源文档 Blob,包括 PDF;远端修订需要显式重新挂载。作者、文件夹或标签关系、保存检索和空间白板也不在当前 WebDAV 同步范围内。空间白板目前通过整库 JSON 导出/导入迁移,该备份同样不包含 PDF 二进制。AI 使用你自己的模型服务与 API Key(OpenAI 兼容 / Anthropic)。
