...
- **One unified canvas bar**: the bottom floating Dock contains the Library, Details, and Overview entries alongside selection, navigation, creation, grouping, linking, and AI tools. Those three entries open a shared panel from the left; Overview includes content counts, fit-to-view, and the MiniMap. This removes both the independent left rail and the former right-side inspector.
...
- **Timeline and citation-tree tidy**: select at least two same-level `PaperNode`s, then use the Dock, the selection context menu, or `Cmd` / `Ctrl` + `Shift` + `L` to arrange them by publication year or citation structure. Citation-tree layout requires a `cites` relationship among the selected papers and keeps circularly citing papers in the same column.
...
- **Quick semantic relationships and source navigation**: in Select mode, hover or focus a card to reveal magnetic handles on its four sides. Drag from one card to another and choose **Cites**, **Supports**, **Contradicts**, or **Extends** from the inline pills—or press `1`–`4`; `Escape` cancels. Each source → target direction is deduplicated, while the reverse target → source relation remains valid. Custom relationships remain editable in the left Details panel, and excerpt cards retain their source anchor for later navigation. Right-click is reserved for the card menu in Select mode; pan with the middle button, `Space`, or the Pan tool.
