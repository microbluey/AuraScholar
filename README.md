...
- **Unified canvas toolbox**: a persistent left rail consolidates the Library, card/relationship details and editing, and canvas overview into one switchable toolbox. The overview includes content counts, fit-to-view, and the MiniMap, replacing the former right-side inspector without occupying both sides of the canvas.
- **Direct card interactions**: a regular click opens a paper or excerpt in the split reader, opens an idea, AI synthesis, or group in the toolbox editor, and opens a clicked edge in the relationship editor. Right-click, a trackpad two-finger click, or the card `...` button opens contextual actions; `Shift` / `Cmd` / `Ctrl`-click remains dedicated to multi-selection.
