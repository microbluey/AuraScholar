...
- **Infinite research canvas**: place whole papers, PDF excerpts, researcher ideas, and AI synthesis results on one pannable, zoomable dot-grid canvas, with box selection, multi-select, relationships, and collapsible groups.
...
- **Timeline and citation-tree tidy**: select at least two same-level `PaperNode`s, then use the contextual selection bar, the multi-select context menu, or `Cmd` / `Ctrl` + `Shift` + `L` to arrange them by publication year or citation structure. Citation-tree layout requires a `cites` relationship among the selected papers and keeps circularly citing papers in the same column.
...
- **Quick semantic relationships and source navigation**: hover or focus a card to reveal magnetic handles on its four sides. Drag to another card and choose **Cites**, **Supports**, **Contradicts**, or **Extends** from the inline pills—or press `1`–`4`. Dropping on empty canvas keeps a dashed preview and opens a current-canvas target picker searchable by title, metadata, tag, group, or node type; choosing a hidden child expands its collapsed group before linking. No edge is written before semantic confirmation, and `Escape` or a workspace switch cancels safely. Each source → target direction is deduplicated, while the reverse relation remains valid.
- **Source-bounded AI synthesis**: select 2–10 paper or excerpt nodes to generate a methodology matrix, contradiction analysis, research-gap analysis, or concise synthesis. Paper nodes provide metadata and available abstracts—not full PDF text—while excerpt nodes provide selected source text. Results retain source nodes and provenance edges; before an asynchronous result is committed, AuraScholar revalidates the request, workspace, and every source node's identity, type, and content version so a stale result cannot create orphan provenance edges. Actual generation requires a configured AI provider.
