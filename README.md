...
- **Crash-safe local note drafts**: the focused Markdown editor automatically keeps workspace- and node-scoped, per-window revisions on the current device, so an unsaved idea note can be recovered after a reload or unexpected exit without two tabs overwriting each other. Closing or navigating away with changes offers **Save and close**, **Discard**, and **Continue editing**; restore and save paths revalidate the workspace, node, and base content version so a newer note is never silently overwritten. These recovery drafts are intentionally device-local and are not synchronized through WebDAV.
...
- **Timeline and citation-tree tidy**: select at least two same-level `PaperNode`s, then use the contextual selection bar, the multi-select context menu, or `Cmd` / `Ctrl` + `Shift` + `L` to arrange them by publication year or citation structure. Citation-tree layout is available only when the selected papers already carry system/legacy citation data in the canvas document; manual links are never classified into that data. Circularly citing papers stay in the same column.
...
- **Highlight-to-canvas workflow**: save a PDF highlight, then drag its excerpt chip onto the canvas or use **Add to current canvas**. AuraScholar creates an `ExcerptNode` with a source link from its `PaperNode`; workspace, paper, source-node, and request identity checks prevent a stale reader operation from writing into another canvas.
