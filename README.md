...
- **Close-safe local persistence**: the desktop app stores the canvas in SQLite, waits for the latest snapshot before explicit route exits, and uses a bounded renderer/main-process save handshake before closing the window or quitting. A failed save keeps the app open unless the user explicitly chooses to force-close. Whole-library JSON backups include canvas data, while Spatial Canvas is not yet included in row-level WebDAV sync. Local PDFs are intentionally unavailable in the browser preview, so the split reader's real PDF workflow must be used in the desktop app. See the [Spatial Canvas product and architecture notes](./docs/SPATIAL_CANVAS_PRD.md) (Chinese).
