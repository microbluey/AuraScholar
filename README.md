...
- **Direct card interactions**: a regular click opens a paper or excerpt in the split reader and opens an idea, AI synthesis, or group in the left Details panel. A click selects a link; double-clicking it edits its optional free-text label inline, with `F2` as the keyboard equivalent. Right-click, a trackpad two-finger click, or the card `...` button opens contextual actions; `Shift` / `Cmd` / `Ctrl`-click remains dedicated to multi-selection.
